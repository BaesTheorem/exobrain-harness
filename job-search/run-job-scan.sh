...
3. Run these lanes, rotating keyword angles day-to-day so the pattern looks human:
   ...
   b2. NICHE BOARDS scripted lane: python3 job-search/nicheboards.py "<angle1>" "<angle2>" --days 2 (two days, not three: Himalayas runs over 2,000 rows a day, a 3-day window truncates at the 6,000-row cap, and the scan is daily so a 2-day window still overlaps). Direct data paths for Himalayas/Remotive/WWR/BuiltIn -- never Google X-ray these boards. SURVIVORS: verify on the employer ATS, then the normal note pipeline. LEADS (comp unlisted): spend a JD read only on squarely in-lane titles; the unlisted-comp DQ still applies after the read.
   ...
   b5. WORKDAY scripted lane: python3 job-search/workday.py -- diffs every pinned + auto-discovered Workday tenant against the last snapshot. Covers the ATS that b3 cannot see (Greenhouse/Lever/Ashby only), and auto-includes any Workday employer already in the tracker, so reposts surface. SURVIVORS arrive pre-gated on all four gates with the comp band parsed from the JD and canApply verified by the ATS itself, so they go straight to status-aware dedup and a listing note -- the detail call already fetched the full JD for the archive callout. LEADS have no comp stated in the JD: apply the usual unlisted-comp judgment call. Report polled/failed/baselined counts in the hub log; a first poll of a board is a baseline, not a dry lane. A line starting with ~ is the crawl'\''s own coverage statement (TRUNCATED at the page cap or the deadline, or caught up with the snapshot): carry every TRUNCATED line into the hub log verbatim. A ** REPOST ** tag or the REPOSTS section is a re-apply candidate per the skill, not a duplicate.
   b6. MISSOURI TALIFY scripted lane: python3 job-search/talify.py --days 3 -- Missouri'\''s state board (jobs.mo.gov front-end), two passes automatic: remote (standard floor, expected-dry -- the board held ONE remote job at build, never read a 0 as breakage) and KC-metro local (onsite floor, band rule gated client-side off detail pages). Survivors/leads: JD read on the Talify detail page, status-aware dedup, listing note with source: talify. Off-lane KC titles print for Alex'\''s overrule -- carry stand-outs into the hub log, do not note them.
   ...
   d. ATS X-RAY: 1-2 WebSearch queries with the domain passed as the tool'\''s allowed_domains parameter -- ["boards.greenhouse.io","job-boards.greenhouse.io"] for Greenhouse, ["jobs.lever.co"] for Lever -- and a plain query, rotating a responsibility phrase (Entra ID, phishing remediation, access provisioning, M365 admin, compromised account, Intune endpoint, IAM analyst). NEVER put the domain in the query as a site: operator: the engine drops it under load and returns builtin.com / themuse.com results with no error, which is what kept logging this lane as dry (fixed 2026-09-07). Do NOT run Ashby or Workable X-rays -- hiring.cafe covers those ATS inventories. Any result from a domain outside allowed_domains = lane did not run; say so.
