...
5. Create listing notes in Projects/Get new job/Job Listings/ per the skill schema (full frontmatter + raw JD archived verbatim in the collapsible callout):
   ...
   - Every NEAR-MISS that passed gates 1-2 but died on gate 3 or 4 after a full JD read: status skipped, declined: true, plus a "## Why skipped" section quoting the exact failed bar. This is what stops tomorrow from re-reading the same JD. Roles killed by the title pre-filter or before a JD read get no note.
