...
| `promethease-md3/` | Material Design overhaul for Promethease DNA reports. Injects one style+script block before `</body>`, leaving the report's own markup and data untouched, so it is idempotent and byte-exactly revertible. Adds a dark theme, status chips, quick-filter chips with live counts, and a compact card mode. CLI: `apply.py report.html`. |
