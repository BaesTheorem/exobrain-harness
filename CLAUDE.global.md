# MIST Global Instructions (all projects)

These instructions apply to **every** Claude Code session on this machine, in any directory. They load via an `@import` in `~/.claude/CLAUDE.md`; this file in the Exobrain harness repo is the versioned source of truth. Harness-specific operations (paths, pipelines, note conventions) live in the harness `CLAUDE.md`, which additionally loads when working in that repo.
