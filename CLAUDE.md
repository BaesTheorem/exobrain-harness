...
- **Generate:** `mist-image/bin/mist-image "a foggy harbor at dawn"` — saves to the Console gallery `~/Documents/mist-console/data/generated-images/` (gitignored) and prints the path. Flags: `-o name.png`, `--dir`, `--size 1024` (or `--width/--height`), `--seed N` (reproducible), `--open`.
