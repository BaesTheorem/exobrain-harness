...
The Combat card's HP block is the compact `.hpbox` widget (shipped 2.41.0), modeled on D&D Beyond: `Heal`/`Damage` buttons (`#hp-heal`/`#hp-damage`) around a number field (`#hp-delta`) on the left, big `Current / Max` digits centered, `Temp` at the right, "Hit Points" beneath. `applyHpDelta(heal)` does the math: Damage absorbs into Temp HP first (5e RAW) then Current (floored at 0); Heal adds to Current capped at Max; the delta clears after applying so a stray double-click can't silently re-apply. **The underlying ids `#hpcur`/`#hpmax`/`#hptmp` are unchanged** — that's what keeps `save()`, `compute()`, `updateDeathSaves()` (death-save row shows at ≤0), rests (`restHp`), and the Classic overlay sync working without touching them; preserve those ids in any future HP rework. Buttons are wired through the delegated `document.addEventListener("click", …)` block (near the ammo/cast handlers), not inline.

**Input-styling gotcha (bit me here):** global `input,select,textarea` (≈line 139) AND `body.dark input` (≈line 47) set border+background on every input, and a plain class rule does NOT out-specify the dark one. To make a borderless/box-free number field (the big HP digits) you need `border:none !important; background:transparent !important` on the class PLUS a `body.dark .hp-big{…!important}` override. Separately, `input[type=number]{appearance:textfield}` (≈line 145) hides spinners on ALL number inputs — to bring spinner arrows back on one field (the `#hp-delta` stepper) set `appearance:auto; -moz-appearance:number-input` + the `::-webkit-*-spin-button{appearance:auto;opacity:1}` pseudos.
