...
1b. **Git history** -- `python3 scripts/exposure_audit.py --history [--repo PATH]`. Checks 1-4 all look at *current* content, so a credential committed once and deleted in the next commit passes every one of them while staying readable forever in the object store. This mode walks every blob reachable from every ref, including branches that were never merged. Run it **before making a repo public** and before any first push to a new remote, not on every commit (it is a whole-history walk: ~14s over 592 commits). A finding here is not fixed by deleting the file; it needs a history rewrite (`git filter-repo`) plus rotating the exposed credential, and rotation is the part that actually matters since anyone may already have a clone.
