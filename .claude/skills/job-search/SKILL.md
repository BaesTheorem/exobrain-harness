...
14. **Missouri Talify** (ADDED 2026-08-26, Alex-provided URL) -- run `python3 "Exobrain harness/job-search/talify.py" --days 3` every discovery pass. Missouri's state talent board at https://missouri.talify.com/jobs (the jobs.mo.gov front-end; apply flows route through app.jobs.mo.gov). Rails/Turbo, server-rendered: the data path is `/jobs.json`, which returns `{"html": <20 job cards>, "next_page": N}` and accepts Ransack params (`q[work_mode_in][]`, `q[employment_type_in][]`, `q[posted_within_days]`, `q[compensation_min_gteq]` + `q[compensation_type_eq]`, `q[keyword_search_cont]`). Facts measured 2026-08-26, baked into the script:
    ...
    - **`compensation_max_gteq` is silently IGNORED** (not Ransack-whitelisted) and returns the unfiltered board -- the failure surfaced as "Food Service Worker" clearing a $103K filter, and the bogus count exactly matched the probe's own pagination cap. Only `min_gteq` works, so the band rule runs client-side: the script pre-filters at a reduced server-side min ($65K) to catch straddling bands, then gates each detail page's band top at the real floor. Known caps (printed in the output): comp-unlisted jobs and bands whose bottom is under the pre-filter are invisible to the local pass.
    - **The board is local-first; that is the lane's value.** At build time the entire board held exactly 1 remote job vs ~1,400 postings in 7 days, so the REMOTE pass (standard floor) is expected-dry -- never read that as breakage. The yield lane is the LOCAL pass: KC-metro seats (zip 640xx/641xx, MO side) gated at the onsite floor, same shape as `usajobs.py`'s local pass. Honest expectation there too: the >=$103K KC pool is mostly State of Missouri trades/corrections/social-services seats, so expect thin IT yield; off-lane KC titles that cleared the comp pre-filter are listed in the output for Alex to overrule.
