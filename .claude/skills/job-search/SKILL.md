...
3. **Google search via WebSearch** (NEW -- added 2026-05-19 after Alex flagged LinkedIn-only blind spot):
   - **X-ray search ATS boards -- pass the domain as the tool's `allowed_domains` parameter, never as a `site:` operator** (fixed 2026-09-07). Call WebSearch with `allowed_domains: ["boards.greenhouse.io", "job-boards.greenhouse.io"]` (Greenhouse hosts on both) or `["jobs.lever.co"]`, and a plain query such as `"access provisioning" remote security analyst`. A `site:` operator inside the query string is a hint the engine drops under load, and when it drops it the results come back from builtin.com / themuse.com / itjobswatch.co.uk with no error at all -- that wrong-domain false negative is what kept logging this lane as dry. `allowed_domains` is a hard filter applied by the tool. Positive control from the day it was fixed: the same phrase returned 5-of-6 off-domain via `site:` and 10-of-10 on-domain via `allowed_domains`, including three in-lane analyst reqs (Vectra, AlertMedia, CompTech) the `site:` form never showed. These often surface listings not crossposted to LinkedIn.
...
10. **Niche boards via direct data paths** (ADDED 2026-08-14, replacing the Google X-ray of these boards) -- run `python3 "Exobrain harness/job-search/nicheboards.py" "<angle1>" "<angle2>" --days 3` every discovery pass. One script, four boards, no search engine in the loop. Verified data paths (2026-08-14):
   ...
   The script pre-applies the gates on structured fields (band rule included) and the title pre-filter, then prints three buckets: **survivors** (gated pass -- verify on the employer ATS like any aggregator hit), **leads** (pass except comp unlisted -- a JD read decides, per the unlisted-comp rule), and declines. Himalayas is a firehose (measured 2026-09-07: over 2,000 rows a day, so a 3-day window overruns the 6,000-row cap): the daily scan runs `--days 2`, which still overlaps day to day, and a `TRUNCATED` warning is a real coverage gap, not noise. Its API sits behind **Cloudflare rate limiting** (measured 2026-09-07: ~100 requests in a 10s window returns 429 `Cf-Mitigated: challenge`, no `Retry-After`, clears in ~25s), which is what the lane's earlier "read operation timed out" deaths actually were. The script paces at 5 concurrent pages with a 1s pause per wave and waits out a 429; a page that still fails prints as a `COVERAGE GAP` with its offset. Do not raise the wave size, and read a `COVERAGE GAP` line as exactly that.
...
12. **USAJOBS** (ADDED 2026-08-14) -- run `python3 "Exobrain harness/job-search/usajobs.py" "<angle1>" "<angle2>" --days 7` every discovery pass. The official federal API (`data.usajobs.gov/api/search`), free but keyed: `USAJOBS_API_KEY` + `USAJOBS_EMAIL` in the harness `.env` (request at https://developer.usajobs.gov/apirequest/). Without a key the script prints instructions and exits 0 -- report the lane as skipped-with-reason, never as failed. Why this lane earns its slot: federal remote IT/security postings mostly never reach commercial boards, many accept experience in lieu of a degree, and Public Trust eligibility (which Alex has) is a common bar. The script runs **two passes per query** (Alex standing instruction 2026-08-14): a nationwide REMOTE pass gated at the standard floor, and a LOCAL pass (Kansas City, MO + 30mi radius) gated at Alex's **onsite floor** -- his onsite floor is binary on any office requirement, one day a week or five, so the whole local pass gets the higher bar (both values live in the script constants; the onsite rationale in the `feedback_onsite_floor` memory). Local survivors are onsite/hybrid federal seats in his home metro, no relocation. Comp is gated mechanically after annualizing hourly bands. Two API quirks baked into the script: `PositionSchedule` Name carries tour-of-duty prose ("Monday-Friday 8:00am...") so full-time is gated on schedule **Code 1**, and keyword matching is loose enough to return transportation roles for "information technology", so an in-lane KEEP title filter applies. Federal postings close on **hard deadlines** and usually want the USAJOBS-profile resume format -- capture the close date on the note and flag anything closing inside 14 days.
   ...
   **Honest expectation for this lane (measured 2026-08-14, key live):** the federal remote market has collapsed under the return-to-office mandate -- `RemoteIndicator=True` with NO other filters returned **32 postings across the entire federal government** (vs 10,000+ unfiltered), nearly all physicians/radiologists/immigration judges, zero IT-series. The param was instrument-checked (positive control: same query without it returned 146 for one keyword), so a 0-survivor day here is the market, not a bug. Keep the lane (one API call, and agency exceptions or a policy change would reopen it overnight); just never read its dryness as breakage, same as the 80k IT-lane expectation. **The LOCAL pass's measured expectation was reset on 2026-09-10 when the onsite floor dropped from $103K to $80K, and the old conclusion no longer holds.** The 2026-08-14 read-through (4-for-4) found the squeeze was structural: postings clearing $103K in KC were GS-13/14, whose Selective Placement Factors and conjunctive specialized-experience blocks are senior-specialist walls Alex fails honestly, while the grades he could actually clear (GS-9/11/12 customer-support 2210s) topped out under the floor. **An $80K floor moves that mid-grade 2210 band back into scope**, which is exactly the tier whose requirements he meets, so this lane should be expected to yield more now, not less. The new composition is **unmeasured** -- do not carry the old "thin local yield" expectation forward as if it still applied, and do not substitute remembered GS locality figures for the band the posting actually prints. Re-read the first few local survivors properly and record what the pool now looks like.
...
13. **Workday-direct boards** (ADDED 2026-08-26) -- run `python3 "Exobrain harness/job-search/workday.py"` every discovery pass. Sibling of lane 11 against the ATS that lane cannot see: `ats-watchlist.py` covers Greenhouse/Lever/Ashby, but a large share of mid-market and enterprise employers host on Workday, and those postings frequently never crosspost. Every Workday tenant exposes an unauthenticated JSON endpoint behind the SPA:
    ...
    **Crawl bounds and what the output lines mean** (added 2026-09-07 after an unfiltered auto-discovered board hung the lane). Auto-discovered boards carry no facets, so they are polled whole; CVS Health reports `total=19016` at ~0.75s a page, and a whole-tenant crawl of ~950 sequential requests hung the lane past the 40-minute scan timeout with zero bytes of output. Boards are newest-first (verified across pages on wd1 and wd5 tenants), so the diff only needs the front of the list: `poll()` caps at `MAX_PAGES` (100 = 2,000 postings), caps wall clock at `BOARD_DEADLINE` (120s), and on an incremental poll of an unfiltered board stops after two consecutive full pages of req ids the snapshot already holds. Faceted boards (R1 RCM) come back in mixed order and never early-stop. Every early exit prints as a `~ <board> ... TRUNCATED ...` or `... caught up ...` line: **carry TRUNCATED lines into the hub log verbatim**, they are the crawl's own coverage statement. The same probe found two latent bugs: `total` was re-read from every page and wd5 tenants return `total=0` after page one, which had silently truncated every unfiltered board to its newest 40 postings since the lane was built (so the first run after the fix flushed a one-time backlog of older reqs, which the printed `started` dates expose); and a board that failed one run was dropped from the snapshot, turning its next poll into a baseline that lost the postings in between. The snapshot is now a merge with `last_seen` stamps aged out after 45 days. A known req id back at the top of its board with a fresh "Posted Today" label after the snapshot last saw it at 30+ days prints with a `** REPOST **` tag and in a `REPOSTS` section: that is a **re-apply candidate**, not a duplicate. Gate 1 also now declines a remote seat in another country (`Canada - Remote AB`, `(Remote, GBR)`), which the first fixed run let through as 1 of 3 survivors and 9 of 24 leads. `--full` disables the early stop so "re-gates everything" means the whole crawl to the cap.

14. **Missouri Talify** (ADDED 2026-08-26, Alex-provided URL) -- run `python3 "Exobrain harness/job-search/talify.py" --days 3` every discovery pass. Missouri's state talent board at https://missouri.talify.com/jobs (the jobs.mo.gov front-end; apply flows route through app.jobs.mo.gov). Rails/Turbo, server-rendered: the data path is `/jobs.json`, which returns `{"html": <20 job cards>, "next_page": N}` and accepts Ransack params (`q[work_mode_in][]`, `q[employment_type_in][]`, `q[posted_within_days]`, `q[compensation_min_gteq]` + `q[compensation_type_eq]`, `q[keyword_search_cont]`). Facts measured 2026-08-26, baked into the script:
    - **The comp threshold normalizes across pay types** -- annually=103000, hourly=49.5, and monthly=8583 return the identical result set, so one annually-typed query covers hourly- and monthly-listed jobs. Comp is still re-verified client-side from each survivor's detail page (detail pages are HTML-only; `/jobs/<id>.json` answers 406).
    - **`compensation_max_gteq` is silently IGNORED** (not Ransack-whitelisted) and returns the unfiltered board -- the failure surfaced as "Food Service Worker" clearing a $103K filter, and the bogus count exactly matched the probe's own pagination cap. Only `min_gteq` works, so the band rule runs client-side: the script pre-filters at a reduced server-side min ($50K, lowered from $65K on 2026-09-10 alongside the floor) to catch straddling bands, then gates each detail page's band top at the real floor. Keep the pre-filter well under the floor: it only exists so a band whose *bottom* misses the floor but whose *top* clears it still surfaces, and raising it toward the floor silently discards exactly those. Known caps (printed in the output): comp-unlisted jobs and bands whose bottom is under the pre-filter are invisible to the local pass.
    - **The board is local-first; that is the lane's value.** At build time the entire board held exactly 1 remote job vs ~1,400 postings in 7 days, so the REMOTE pass (standard floor) is expected-dry -- never read that as breakage. The yield lane is the LOCAL pass: KC-metro seats (zip 640xx/641xx, MO side) gated at the onsite floor, same shape as `usajobs.py`'s local pass. Honest expectation there was measured at the old $103K floor, where the qualifying KC pool was mostly State of Missouri trades/corrections/social-services seats with thin IT yield. **The floor dropped to $80K on 2026-09-10 and that pool has not been re-measured** -- it is materially larger now, so treat the composition as unknown rather than assuming the old thin-IT read still holds. Off-lane KC titles that cleared the comp pre-filter are listed in the output for Alex to overrule.
...
| 3 | Greenhouse / Lever X-ray | | One query per board, with the domain in `allowed_domains`, never in a `site:` operator (Source #3). Do **not** `OR` two boards into a single query -- the engine drops one half silently. |
...
2. **Google/WebSearch discovery scan** (added 2026-05-19; NARROWED 2026-08-14 -- the niche boards moved to their direct data paths in `nicheboards.py`, so never X-ray Himalayas/BuiltIn/WWR/Remotive anymore):
   - Rotate 1-2 Google X-ray searches per day across the ATS boards only. Suggested rotation (alternate which to skip):
     - WebSearch with `allowed_domains: ["boards.greenhouse.io", "job-boards.greenhouse.io"]` and query `"<responsibility phrase>" remote` -- pick a different responsibility phrase each day (Entra ID, phishing remediation, access provisioning, M365 admin, compromised account, Intune endpoint, etc.)
     - WebSearch with `allowed_domains: ["jobs.lever.co"]` and the same query shape
     - **Never put the domain in the query as `site:`** -- see Source #3 for why (the engine drops the operator under load and returns other domains with no error). Ashby and Workable X-rays stay retired; hiring.cafe covers those inventories.
...
2b2. **Niche boards + ATS watchlist + USAJOBS scripted lanes** (ADDED 2026-08-14 -- see Sources #10-12 for the per-board gotchas):
   - `python3 "Exobrain harness/job-search/nicheboards.py" "<angle1>" "<angle2>" --days 2` -- rotate angles like the other lanes; two days because the Himalayas firehose overruns the row cap at three (Source #10). Survivors: verify on the employer ATS, then the normal note pipeline. Leads (comp unlisted): spend a JD read only when the title is squarely in-lane; the unlisted-comp DQ still applies after the read.
