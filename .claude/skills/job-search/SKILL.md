...
Some employers get scanned directly on every discovery pass because Alex has an inside referral path there — a warm intro is worth more than cold volume, so these clear a lower bar than the open market.

- **Black & Veatch** — careers portal `https://careers.bv.com` (JS-rendered SuccessFactors; search `https://careers.bv.com/search/?q=<keyword>`, also Google X-ray `site:careers.bv.com <keyword>`, and confirm open by absence of a "This position has been filled" banner on the job page). B&V is a KC-HQ (Overland Park) engineering/construction firm, so its corporate IT/security org is thin and mostly senior or OT/ICS-engineering — filter hard for entry-accessible corporate-IT/security/identity/GRC roles and skip the SCADA/substation/automation engineering and non-US (Pune) postings. **Watch reason**: Alex has a warm internal referral contact in B&V's security org who forwarded his resume toward the (deputy) CISO and offered to be named as a referral. Contact name, the email thread, and the comp the contact quoted live in the vault — see the People note and `Projects/Get new job/Claude Reference.md` (do not inline those private details here). Standing tasks to keep warm: the CISO-referral follow-up + "scan B&V listings for a fresh opening to anchor a re-message."
  - **Remote-gate exception (IMPORTANT)**: B&V is the documented exception to hard requirement #1 (fully remote). Its model is 90 days fully in-office then 3 days/week hybrid at Overland Park. Because the firm is in Alex's home metro (he's in Midtown KC, no relocation) AND it's a warm referral AND the contact confirmed comp clears Alex's floor, Alex has explicitly opted in to the hybrid arrangement here. So **do not auto-DQ B&V roles for being hybrid/in-office** — apply the other three gates (full-time permanent, comp ≥$75K or strong inference, ≥80% strong fit) normally. The exception is B&V-specific and connection-specific; it does not generalize to other hybrid employers. Watch the actual reporting location, though: some B&V "OnSite" roles report to out-of-state project sites (e.g. a Beech Island, SC cyber-custodian role requiring 12+ months on-site), which is relocation, not local-hybrid — flag those.
