...
- From `~/restore-staging/repos-gitignored/<repo>/`: phone GCP keys, disposable-email secrets, tv/token.json, etc. -- rsync these in Phase 5 *after* each repo is cloned, not before, or the clone fails on a non-empty dir.
